...
static volatile hand_data hand;
...
void *handle_client(void *args)
{
	...
	*space = '\0';
	...
	if (recvbuf[5] == 'x')
	{
	  get_gyro_orientation((hand_data *)&hand);
	  char orientation[40];
	  sprintf(orientation, "%ddeg %ddeg %ddeg", (int)(hand.data[hand_roll] + 0.5), (int)(hand.data[hand_pitch] + 0.5), (int)(hand.data[hand_yaw] + 0.5));
...
static int webserver_tcpport;


void *webserver(void *args)
{
    int listenfd = 0;
    ...
    serv_addr.sin_port = htons(webserver_tcpport);
    ...
    close(listenfd);
    printf("webserver terminated.\n");
    return 0;
}

int init_webserver(int tcpport)
{
    webserver_tcpport = tcpport;
    pthread_t tid;

    if (pthread_create(&tid, 0, webserver, 0))
    {
        logmsg("pthread_create returned error");
        return 0;
    }
