...
int main(int argc, char * argv[]) try
{
    ...
    float       alpha = 0.8f;               // Transparancy coefficient
	...
	printf("depth size: %d, color size: %d\n", depth.get_data_size(), color.get_data_size());
	printf("depth: %d x %d, color: %d x %d\n", 
		depth_width, depth_height, color_width, color_height);

	int brick_width, brick_height, brick_size, brick_row, brick_col;

	buffer = (uint8_t *) color.get_data();
	sirka = color_width;
	vyska = color_height;
        najdi_loptu(&brick_width, &brick_height, &brick_size, &brick_row, &brick_col);

	printf("brick: [%d,%d]  %d x %d  (%d pix)\n",
			brick_col, brick_row, brick_width, brick_height,
			brick_size);
